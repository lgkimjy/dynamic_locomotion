...
#include "polynomial_end_effector_trajectory.hpp"
    ...
    const double& end_time)
{
    ...
    // std::cout << time_vec_z_.transpose() << std::endl;
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        B_ineq_(i + n) = std::max(start_pose(2), target_pose(2)) + 1.0 * mid_air_height_;
    ...
    {
        std::cout << GREEN << "Online Swing Trajectory Optimal Solution solved" << RESET << std::endl;
    }
    return true;
}
    ...
    Eigen::Ref<Eigen::Vector3d> next_acceleration)
{
    ...
    // std::cout << "next_pose: " << next_pose.transpose() << std::endl;
}
